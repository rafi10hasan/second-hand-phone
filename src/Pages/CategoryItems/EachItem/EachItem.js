...
const EachItem = ({ product, setProductItem }) => {
  ...
  //   queryKey: [sellerMail],
  //   queryFn: async () => {
  //     const res = await fetch(`https://second-hand-phone-server.vercel.app/users/${sellerMail}`);
  ...
  // useEffect(() => {
  //   fetch(`https://second-hand-phone-server.vercel.app/users/${sellerMail}`)
  //     .then((res) => res.json())
  //     .then((data) => {
  ...
  const handleReportProduct = (product) => {
    ...
    fetch(`https://second-hand-phone-server.vercel.app/report/${product._id}`, {
      method: "PATCH",
      headers: {
  ...
  return (
    <div className="card w-full bg-base-100 shadow-2xl">
      ...
      <div className="card-body items-center text-center">
        ...
        <button onClick={() => handleReportProduct(product)} className="btn btn-warning">
          Report to admin
        </button>
      </div>
    </div>

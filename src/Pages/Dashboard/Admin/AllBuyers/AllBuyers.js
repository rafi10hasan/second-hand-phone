...
const AllBuyers = () => {
  ...
  useEffect(() => {
    axios
      .get("https://second-hand-phone-server.vercel.app/lists/buyer")
  ...
  const handleDeleteBuyer = (buyer) => {
    ...
    fetch(`https://second-hand-phone-server.vercel.app/users/${buyer._id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
          ...
          <tbody>
            {buyers.map((buyer, i) => (
              <tr key={i}>
                ...
                <td>{buyer?.email}</td>
                <td>
                  <button onClick={() => handleDeleteBuyer(buyer)} className="btn btn-warning">
                    Delete
                  </button>
                </td>
              </tr>

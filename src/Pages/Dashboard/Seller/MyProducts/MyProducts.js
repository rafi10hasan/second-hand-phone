...
const MyProducts = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://second-hand-phone-server.vercel.app/products/${user?.email}`
      );
  ...
  const handleDeleteProduct = (myProduct) => {
    ...
    fetch(`https://second-hand-phone-server.vercel.app/products/${myProduct._id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.deletedCount > 0) {
          refetch();
          toast.success(`Product ${myProduct.productName} deleted successfully`);
        }
      });
  };
  ...
  const handleAdvertiseProduct = (myProduct) => {
    const confirmAdvertise = window.confirm(`Advertise ${myProduct.productName} ??`);
    if (!confirmAdvertise) {
              ...
              <tr key={i}>
                ...
                <td>
                  {myProduct.advertised ? (
                    <button className={`btn btn-success ${myProduct.sold && "btn-disabled"}`}>
                      Advertising Now
                    </button>
                  ) : (
                    <button
                ...
                <td>
                  <button className="btn btn-error" onClick={() => handleDeleteProduct(myProduct)}>
                    Delete
                  </button>
                </td>
              </tr>
